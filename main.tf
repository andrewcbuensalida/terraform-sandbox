...
# data is for reads, and in this case, is used to get an existing key pair. the aws in aws_key_pair is the provider, the key_pair is the type of resource, ec2-key-pair-portfolio-1 is a custom name used only in these files. or could just put string "ec2" in key_name in instance resource
